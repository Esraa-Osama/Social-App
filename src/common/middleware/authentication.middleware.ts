...
export const decodeTokenAndFetchUser = async (authorization: string) => {
  if (!authorization) {
    throw new APPError(
...
export const authentication = async (
  ...
  const { user, decoded } = await decodeTokenAndFetchUser(authorization!);
  req.user = user;
  req.decoded = decoded;
  next();
};
...
export const authenticationGQL = async (authorization: string) => {
  return await decodeTokenAndFetchUser(authorization!);
};
